...
def miou(logits, targets, eps=1e-6):
	...
	#scatter可以用来改变张量数据，参数用法，dim=1表示改变该维度的值，具体位置多少就是index里面的数据，具体值是后面的value
	outputs = torch.zeros_like(logits).scatter_(dim=1, index=outputs.type(torch.int64), value=1).type(torch.int8)
	targets = torch.zeros_like(logits).scatter_(dim=1, index=targets.type(torch.int64), value=1).type(torch.int8)
