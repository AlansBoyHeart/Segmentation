...
def dice_loss(logits, targets, smooth=1.0):
	...
	targets = torch.unsqueeze(targets, dim=1)  # type: Any
	targets = torch.zeros_like(logits).scatter_(dim=1, index=targets.type(torch.int64), value=1)
...
def custom_pspnet_loss(logits, targets, alpha=0.4):
	...
	if type(logits)==tuple:
		with torch.no_grad():
			_targets = torch.unsqueeze(targets, dim=1)   #增加维度为1的一个维度
			#functional.interpolate为上下采样方法，size是目标尺寸，mode插值方法，align_corners=True表示对齐 input 和 output 的角点像素(corner pixels)，保持在角点像素的值
