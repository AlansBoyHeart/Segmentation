...
class PyramidPoolingModule(nn.Module):
	def __init__(self, in_channels, pyramids=[1,2,3,6]):   #(512,[1,2,3,6])
		super(PyramidPoolingModule, self).__init__()
		self.pyramids = pyramids
		out_channels = in_channels // len(pyramids)   #out_channels = 128
	...
	def __init__(self, backbone='resnet18', num_classes=2, pretrained_backbone=None):
		...
		if 'resnet' in backbone:
			...
			self.main_output = nn.Sequential(OrderedDict([
				("conv1", ConvBlock(2*stage5_channels, stage5_channels//4, kernel_size=3, padding=1, bias=False)),  #(1024,128)
			...
			self.aux_output = nn.Sequential(OrderedDict([
				("conv1", ConvBlock(stage5_channels//2, stage5_channels//8, kernel_size=3, padding=1, bias=False)),  #(256,32)
				("dropout", nn.Dropout2d(p=self.dropout)),
